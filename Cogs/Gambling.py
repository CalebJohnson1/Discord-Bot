...
class Gambling(commands.Cog, name="Gambling.py"):
    ...
    async def gamble_error(self, ctx, error):
        if isinstance(error, commands.CommandOnCooldown):
            await ctx.send(f"Please wait another {round(error.retry_after, 2)} seconds to roll the dice.")
            return

        await ctx.send("Please specify an amount to gamble! <m!55x2 (amount)>")
        print(error)
